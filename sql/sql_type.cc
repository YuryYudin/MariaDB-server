...
bool Time::to_native(Native *to, uint decimals) const
{
  ...
  if (to->reserve(len))
    return true;
  /*
    Truncate the unpacked MYSQL_TIME to the declared precision before
    packing, mirroring Type_handler_timestamp_common::TIME_to_native().
    Callers may build a Time from an Item whose MYSQL_TIME carries more
    fractional digits than the requested 'decimals'; my_time_packed_to_binary
    asserts the packed value is already truncated, so without this step
    the assertion can fire on otherwise-valid inputs.
  */
